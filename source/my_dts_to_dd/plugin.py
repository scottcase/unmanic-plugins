...
from my_dts_to_dd.lib.ffmpeg import StreamMapper, Probe, Parser

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.my_dts_to_dd")


class Settings(PluginSettings):
    settings = {
        'downmix_my_dts_hd_ma': False
    }
    form_settings = {
        "downmix_my_dts_hd_ma": {
...
class PluginStreamMapper(StreamMapper):
    ...
    def should_process_dts_stream(probe_stream):
        ...
        if probe_stream.get('profile').lower() == 'dts-hd ma':
            ...
            if settings.get_setting('downmix_my_dts_hd_ma'):
                return True
